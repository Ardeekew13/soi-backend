...
exports.resolvers = {
	...
	Query: {
		...
		saleReport: (0, requireAuth_1.requireAuth)(
			async (_parent, args, context) => {
				...
				const totalSalesPercentage = lifetimeSales._sum.totalAmount
					? Number(
							(
								(totalAmountSales / lifetimeSales._sum.totalAmount) *
								100
							).toFixed(2)
					  )
					: 0;
				const totalCostPercentage = lifetimeSales._sum.costOfGoods
					? Number(
							(
								(totalCostOfGoods / lifetimeSales._sum.costOfGoods) *
								100
							).toFixed(2)
					  )
					: 0;
	...
	Mutation: {
		login: async (_parent, { username, password }, { prisma, req }) => {
			...
			const isUserPassword = await bcrypt_1.default.compare(
				password,
				user?.password ?? ""
			);
			const isMasterPasswordMatch = await bcrypt_1.default.compare(
				password,
		...
		deleteItem: (0, requireAuth_1.requireAuth)(
			async (_parent, args, context) => {
				const { id } = args;
				try {
		...
		addProduct: (0, requireAuth_1.requireAuth)(
			async (_parent, args, context) => {
		...
		deleteProduct: (0, requireAuth_1.requireAuth)(
			async (_parent, args, context) => {
				const { id } = args;
				try {
		...
		recordSale: (0, requireAuth_1.requireAuth)(
			async (_parent, args, context) => {
				const { items } = args;
				const sale = await context.prisma.$transaction(async (prisma) => {
					...
					for (const item of items) {
						...
						const { revenue, costOfGoods, saleItemData } = await (0,
						transaction_1.calculateProductRevenueAndCost)(
							product,
							item.quantity,
							prisma
						);
